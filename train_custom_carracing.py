...
# Setup MPS device before importing PPO
def get_device():
...
# Now import PPO with the device already set
try:
    # Force reload the module to ensure it uses our device
    if 'PPO_PyTorch.PPO' in sys.modules:
        del sys.modules['PPO_PyTorch.PPO']
...
class ImagePPO(PPO):
    def __init__(self, state_dim, action_dim, lr_actor, lr_critic, gamma, K_epochs, eps_clip,
        ...
        # Create CNN encoder and move to device
        self.encoder = CNNEncoder(encoder_dim).to(device)
        
        # Make sure policy networks are on the correct device
        self.policy = self.policy.to(device)
        self.policy_old = self.policy_old.to(device)
    ...
    def update(self):
        ...
        # Encode all states in buffer
        old_encoded_states = []
        for state in self.buffer.states:
            state_tensor = torch.FloatTensor(state).unsqueeze(0).to(device)
            encoded_state = self.encoder(state_tensor)
            old_encoded_states.append(encoded_state)
        
        old_encoded_states = torch.cat(old_encoded_states, dim=0).detach()
        
        # Convert other buffer items to tensors
        ...
        for _ in range(self.K_epochs):
            # Get new action probabilities and state values with the current policy
            logprobs, state_values, dist_entropy = self.policy.evaluate(old_encoded_states, old_actions)
        ...
        return {
            'actor_loss': actor_loss_total / self.K_epochs,
            'critic_loss': critic_loss_total / self.K_epochs, 
            'entropy': entropy_total / self.K_epochs
        }
