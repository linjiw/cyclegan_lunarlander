...
# Modified hard environment - more balanced challenge
gym.register(
    id="CustomMountainCarHard-v0",
    entry_point=CustomMountainCar,
    max_episode_steps=1000,
    kwargs={
        'force': 0.0015,        # Moderately weaker engine (increased from 0.0008)
        'gravity': 0.002,       # Moderate gravity (decreased from 0.003)
        'max_speed': 0.08,      # Moderate max speed (increased from 0.06)
        'goal_position': 0.5,   # Standard goal position (decreased from 0.55)
        'init_position_low': -0.8,  # Less extreme starting position (changed from -1.1)
        'init_position_high': -0.6  # Less extreme starting position (changed from -0.9)
    }
)
    ...
    while i_episode < max_episodes:
        ...
        for t in range(max_timesteps):
            ...
            if time_step % update_timestep == 0:
                ...
                ppo_agent.update()  # No return value from update()
                
                # Just log that an update occurred
